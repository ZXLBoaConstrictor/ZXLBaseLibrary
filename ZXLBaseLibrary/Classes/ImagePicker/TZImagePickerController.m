...
- (void)viewDidLoad {
    ...
    // 默认的外观，你可以在这个方法后重置
    self.oKButtonTitleColorNormal   = [UIColor colorWithRed:(255/255.0) green:(164/255.0) blue:(47/255.0) alpha:1.0];
    self.oKButtonTitleColorDisabled = [UIColor colorWithRed:(255/255.0) green:(164/255.0) blue:(47/255.0) alpha:0.5];
...
- (void)setPhotoPreviewMaxWidth:(CGFloat)photoPreviewMaxWidth {
    ...
    if (photoPreviewMaxWidth > 1200) {
        _photoPreviewMaxWidth = 1200;
...
- (void)setAllowPickingImage:(BOOL)allowPickingImage {
    ...
    [[NSUserDefaults standardUserDefaults] setValue:allowPickingImageStr forKey:@"tz_allowPickingImage"];
...
- (void)setAllowPickingVideo:(BOOL)allowPickingVideo {
    ...
    [[NSUserDefaults standardUserDefaults] setValue:allowPickingVideoStr forKey:@"tz_allowPickingVideo"];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
...
- (void)configTableView {
    ...
    [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage completion:^(NSArray<TZAlbumModel *> *models) {
        ...
        if (!self->_tableView) {
            ...
            self->_tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, top, self.view.tz_width, tableViewHeight) style:UITableViewStylePlain];
