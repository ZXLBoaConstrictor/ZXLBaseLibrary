...
@interface TZVideoPlayerController () {
    ...
    UIView *_toolBar;
    UILabel *_videoTimeTipLab;
    UIButton *_doneButton;
    UIProgressView *_progress;
    
    UIStatusBarStyle _originStatusBarStyle;
    
    NSInteger   _maxVideoTime; // 限制视频的时间
}
@end
...
- (void)configMoviePlayer {
    ...
    [[TZImageManager manager] getVideoWithAsset:_model.asset completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
        if ([[info objectForKey:PHImageResultIsInCloudKey] boolValue] && ![info objectForKey:@"PHImageFileSandboxExtensionTokenKey"]) {
            [SVProgressHUD showWithStatus:@"视频从iCloud加载中..."];
            [[TZImageManager manager] getVideoOutputPathWithAsset:self->_model.asset completion:^(NSString *outputPath) {
                if (outputPath != nil && outputPath.length >0) {
                    [SVProgressHUD dismiss];
                    dispatch_async(dispatch_get_main_queue(), ^{
                        self->_player = [AVPlayer playerWithURL:[NSURL fileURLWithPath:outputPath]];
...
- (void)configBottomToolBar {
    _toolBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.tz_height - 44 - iOSSafeAreaBottomHeight, self.view.tz_width, 44 + iOSSafeAreaBottomHeight)];
    ...
    [self isShowTimeLabAndDoneBtn];
}

//warning TODO 修改源码 添加时间提示Label
- (void)setupTimeLab
{
    //修改源码 限制最长的时间（改为5分钟）
    _maxVideoTime = 300;
    
    _videoTimeTipLab = [[UILabel alloc] initWithFrame:CGRectMake(10, 0, self.view.tz_width - 44 - 12, 44)];
    _videoTimeTipLab.textColor = [UIColor whiteColor];
    _videoTimeTipLab.text = [NSString stringWithFormat:@"视频时间超出了%zd分钟", _maxVideoTime / 60];
    [_toolBar addSubview:_videoTimeTipLab];
}

//warning TODO 修改源码 视频时间超出5分钟,提示文字,并隐藏“完成”按钮
- (void)isShowTimeLabAndDoneBtn {
    
    if ([self getVideoTimeCount] > _maxVideoTime)
    {
        _videoTimeTipLab.hidden = NO;
        _doneButton.hidden = YES;
    }else
    {
        _videoTimeTipLab.hidden = YES;
...
//warning TODO 修改源码 获取视频时长
- (NSInteger)getVideoTimeCount
{
    
    NSArray *timeArray = [_model.timeLength componentsSeparatedByString:@":"];
    
    NSInteger timeCount = [timeArray[timeArray.count - 2] integerValue] * 60 + [[timeArray lastObject] integerValue];
